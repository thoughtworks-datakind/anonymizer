...
class TestReportGenerator(TestCase):
    ...
    def test_calculate_detector_counts_skims_through_a_column_and_counts_detector_results_in_each_of_them(self):
        result_column_values = pd.Series(np.array([AnalyzerResult("S0000001I", "NRIC", 38, 47), AnalyzerResult("test@sample.com", "EMAIL", 45, 60), AnalyzerResult("test@sample.com", "EMAIL", 45, 60)]))
